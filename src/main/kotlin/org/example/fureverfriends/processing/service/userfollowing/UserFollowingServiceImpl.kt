...
class UserFollowingServiceImpl(
    ...
    override fun findFollowings(currentUsername: String, index: Int): org.example.fureverfriends.api.dto.user.FoundUsersDTO {
        ...
        return org.example.fureverfriends.api.dto.user.FoundUsersDTO(
            foundUsers = followingPage.content.map { it.following.mapToDTO() }, isLastPage = followingPage.isLast
        )
    }

    override fun findFollowers(currentUsername: String, index: Int): org.example.fureverfriends.api.dto.user.FoundUsersDTO {
        val pageable = PageRequest.of(index, pageSize)
        val followingPage = userFollowingRepository.findUserFollowingsByIdFollowingAndStatus(currentUsername, ACCEPTED, pageable)
        return org.example.fureverfriends.api.dto.user.FoundUsersDTO(
            foundUsers = followingPage.content.map { it.follower.mapToDTO() }, isLastPage = followingPage.isLast
        )
    }
}